...
def cdf(t, m = 0, b = 0, q = 0):
    ...
    try:
        ...
        #print(f"Error computing -log(b) with b={b}: {e}")
        g = -float("inf")
    try:
        l = math.log(1 + b)
    except ValueError as e:
        #print(f"Error computing log(1+b) with b={b}: {e}")
        l = float("inf")
    k = 1 - b
    try:
        negs = math.exp((r * g) - l + math.log(b + ((t - r + (1/2)) * k)))
    except OverflowError as e:
        #print(f"Error computing negs: {e} r={r}, g={g}, l={l}, b={b}, t={t}, k={k}")
        negs = float("inf")

    try:
        poss = 1 - math.exp((r * (-g)) - l + math.log(b + ((r - t + (1/2)) * k)))
    except OverflowError as e:
        #print(f"Error computing poss: {e} r={r}, g={g}, l={l}, b={b}, t={t}, k={k}")
        poss = float("inf")

    # Check for infinities
    if math.isinf(negs):
        negs = 0
        #print(f"negs was infinite, set to 0. r={r}, g={g}, l={l}, b={b}, t={t}, k={k}")
    if math.isinf(poss): 
        poss = 0
        #print(f"poss was infinite, set to 0. r={r}, g={g}, l={l}, b={b}, t={t}, k={k}")
